...
def main():
    ...
    human_metrics = train_and_evaluate("human", human_seed, metric_type="MCC", device=device)
    pd.DataFrame([human_metrics]).to_csv("./reproduce_results/human_reproduce.csv", index=False)
    print("[INFO] Human results saved -> reproduce_results/human_reproduce.csv")
